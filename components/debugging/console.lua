...
function updateDebug(dt)
	...
	if keyPressed["BACKSPACE"] then
		...
		debugCursorPosition = _G._G.math.max(debugCursorPosition - 1, 0)
		debugCursorBlink = 0
	end
	...
	if keyPressed["DELETE"] then
		...
		-- debugCursorPosition = _G._G.math.max(debugCursorPosition, 0)
		debugCursorBlink = 0
	end
	...
	if keyPressed.LEFT or keyPressed.RIGHT and not (keyPressed.LEFT and keyPressed.RIGHT) then
		...
		debugCursorPosition = _G._G.math.min(_G._G.math.max(debugCursorPosition + direction, 0), debugText:len())
		debugCursorBlink = 0
	end
	...
	debugScrollTarget = debugScrollTarget + _G._G.math.min(_G._G.math.max(cursor.wheel, -10), 10) * 64
	...
	debugScrollTarget = _G._G.math.min(_G._G.math.max(debugScrollTarget, scrollLimit), 0)

	local round_padding = debugPadding / 4
	local input_h = debugPadding * 2 + 50 - round_padding * 2 + _G._G.math.max(res.getStringHeight(debugText) - 50, 0)
	...
	x, y = _G._G.math.floor(x), _G._G.math.floor(y)
