...
function drawLayer(layer, yoffset)
	...
	if w > 0 and wScale > 0.02 then --don't draw so many if the scale is too low
		for x = -1, math.floor(screenWidth / (w - px) / wScale) do
			...
			setRenderState(
				pivotX + left - shakeX / (relativeScale + xScale),
				top - shakeY / (relativeScale + yScale),
				wScale * (relativeScale + xScale),
				wScale * (relativeScale + yScale),
				0,
				px,
				py
			)
...
function drawForegroundNative()
	...
	for layernum, layer in ipairs(fgLayers) do
		if layernum == ground_num then
			...
			drawRect(
				theme.groundColor.r / 255,
				theme.groundColor.g / 255,
				theme.groundColor.b / 255,
				1,
				rect_x,
				rect_y,
				screenWidth,
				screenHeight + screenTop * s + rect_y
			)
		end
